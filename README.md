...
```powershell
dotnet add package CSweet.Agent.SDK --version 0.4.1
```
...
Run the batch file from the repository root to restore dependencies, run the test suite, and create the package and symbols package in a versioned directory such as `artifacts\packages\0.4.1`:
...
```bat
Create-NuGetPackages.bat 0.4.1 C:\packages\csweet-agent-sdk
```
