...
SDK 3.46.0 exposes `context.Platform.Compute`. This calls the same authorized broker as the platform's MCP compute tools; agent authors do not implement MCP, provisioning, UAC or credential handling.
...
## Isolated source snapshots (SDK 3.46.0)

Workspace archive size, expanded size, and file-count limits are supplied by the C-Sweet runtime
for each workload. Agent packages do not choose or compile these policy values. Operators can
adjust the matching C-Sweet workspace-sync configuration without rebuilding the SDK or agent.
...
### Requester-controlled lifetime

SDK 3.46.0 accepts `LifetimeSeconds: 0` for compute retained until the requester calls `DestroyAsync` with the current generation and a stable idempotency key. The broker must explicitly grant this lifetime. A positive duration continues to request timed expiry. `LeaseExpiresAt == DateTimeOffset.MaxValue` means until released. Compute retention grants do not imply network access; publishing still requires an explicit instance-specific grant. Do not destroy a delivered test instance merely because its build task completed: retain it until the user is done testing.
